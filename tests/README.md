...
across copies/reassignment/control flow, indexed assignment on mutable arrays,
deferred function-result inference, and
...
operators lower to the expected signed IR opcodes, indexed assignment emits an
element store, and `compile_with_diagnostics()` returns warnings alongside IR.
